...
- 読み込み・描画の進行に伴う**一度きり**の見た目変化: 遅延読み込み画像
  ...
  **しない**——代替字形のまま撮り、警告（フォント名と、外部依存の応答が
  ...
  件数・初出 story・family 名を運ぶ集計の 1 行に畳む（story ごとの
  `story failed` 行と違い、行ごとに新しい情報が無いため）。畳みの代償として
  **2 件目以降の story ID はビルドログに残らない**——どの story が警告を
  出したかを個別に知る手段は無く、残るのは件数と初出 story のみである
  （意図した割り切り）。失敗判定にすると
