...
- 読み込み・描画の進行に伴う**一度きり**の見た目変化: 遅延読み込み画像
  ...
  解決しなければその story は失敗になる（fail-closed）。読み込みに**失敗**した
  フォント（404・接続断等）が 1 つでもある場合もその story は失敗になる——
  `document.fonts.ready` は読み込みの成否に依らず解決するため、断続的にしか
  届かない外部フォントを黙って通すと、同一ビルドから Web Font 版と代替字形版の
  二つの baseline が生まれ「同じビルドから同じ絵」の保証が破れる。失敗の
  メッセージにはフォント名と対処（フォントファイルをビルドへ同梱するか
  `@font-face` 参照を外す）が示される
