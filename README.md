...
- 読み込み・描画の進行に伴う**一度きり**の見た目変化: 遅延読み込み画像
  ...
  解決しなければその story は失敗になる（fail-closed）。検証と撮影の間に
  document が入れ替わった（story が自分を reload した）場合や新たな読み込みが
  始まった場合は、撮影直前の再確認が検知して検証をやり直す——検知できないのは
  再確認から撮影までの最後の一往復に始まる変化だけである。読み込みに
  **失敗**したフォント（404・接続断・`local()` 不在等）は story を失敗に
  **しない**——代替字形のまま撮り、警告（フォント名と対処: フォントファイルを
  ビルドへ同梱するか `@font-face` 参照を外す）をログに残す。失敗判定にすると
