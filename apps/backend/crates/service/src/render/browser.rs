...
//!    preview の render phase（play が走っている間は待ち、終わった phase で
//!    初めて READY とする）を待ち、
...
//!    公開する構成では phase が play の最中（`playing` まで）でない
//!    ことも併せて要求する。`storyErrored` / `storyThrewException` /
...
//! 帰属）を持つ（検知の読みが表に載らず裸のまま残った）。
//! **走査を揃える・門を足すときは、この表の全行に同じ列を検めること。**
//!
...
JSON.stringify((() => {
  ...
  try {
    ...
    if (render) {
      ...
      // `completing` は play を抜けた後の短い区間だが、`completed` だけを
      // ready としていた従来と同じ側（pending）へ倒しておく。
      if (
        phase === 'preparing' || phase === 'loading' || phase === 'beforeEach' ||
        phase === 'rendering' || phase === 'playing' || phase === 'played' ||
        phase === 'completing'
      ) {
        return { state: 'pending', dom_ready: domReady };
      }
      if (phase === 'completed' || phase === 'afterEach' || phase === 'finished') {
        ...
        // doc の常駐状態表を参照）——前 document の完了 phase が差し替えを
        ...
        // ものが 30 秒 Timeout に倒れる。向きは fail-closed（stale な完了 phase
      ...
      // 世代つきの rendered 印だけを見る下の経路へ落とす——名前が一つ
      // 増えただけで全 story が Timeout になる形にはしない。
    }
  } catch (e) {
...
const FONTS_WAIT_BODY: &str = r#"
  ...
  const readyOrDetached = (s) => {
    ...
    const detached = new Promise((resolve, reject) => {
      const check = () => {
        if (settled) return;
        // 検知の読み（defaultView）も裸にしない——throwing
        // getter だと setTimeout コールバック内の uncaught throw で検知が
        // 黙って死に、この promise は永久 pending・story は原因を失った
        // deadline の Timeout に化ける（他の読みと同じ理由）。「切り離し
        // 扱い」へ黙って resolve する形も禁物——読めない document を検証
        // せずに待ちから外し、未検証のまま撮る fail-open になる。reject に
        // 帰属の印（vrtWaitError）を載せて errors へ倒す（fail-closed）。
        let gone;
        try {
          gone = !s.doc.defaultView;
        } catch (e) {
          reject({ vrtWaitError: 'reading document.defaultView threw: ' + String(e) });
          return;
        }
  ...
  const settle = () => {
    ...
    return Promise.all(races).then(
      () => {
        // 切り離しを race が検知した場合は、ここで生きている document だけへ
        // 絞る——外れた document の fonts は待ちからも判定からも外す（読める
      ...
      // 帰属を分ける: 切り離し検知の読みの throw（vrtWaitError の印つき）は
      // その文言のまま、素の reject は fonts.ready のものとして報告する——
      // 印なしで一律 'fonts.ready rejected' と書くと、defaultView getter の
      // throw が別の原因に化ける。
      (e) => JSON.stringify({
        ok: false,
        errors: [e && e.vrtWaitError ? e.vrtWaitError : 'document.fonts.ready rejected: ' + String(e)]
      })
    );
  };
  return settle();
...
fn fonts_verdict(
    ...
    match parsed.get("ok").and_then(|v| v.as_bool()) {
        ...
        Some(true) => {
            ...
            // JS の Set 列挙は挿入順＝走査・宣言順で、同じ集合でも document の
            // 並びや読み込みの進み方で run ごとに変わりうる。警告文字列を
            // 集合として同値な入力へ決定的にするため、Rust 側で sort してから
            // 打ち切る——打ち切りで名の消える family も順に依らず同じになる
            // （Rust 側で閉じ、スクリプト側の列挙順に契約を
            // 求めない）。
...
{on_channel_js}
    ...
    /// 本物の `StoryRender` と同じ phase 順を再現する play fixture。
    ///
    /// SB 9 の `StoryRender` は preparing → loading → beforeEach → rendering →
    /// playing → played → completing → completed → afterEach → finished と進み、
    /// `storyRendered` は **`completed` phase の中で** emit される。`completed`
    ...
    fn write_storybook_phase_order_bundle(root: &Path) {
      ...
      // play の開始で背景を緑へ変える。撮れた絵から play の実行が観測できる。
      render.phase = 'playing';
      window.__VRT_PHASE_ORDER__.push('playing');
      document.getElementById('play-target').style.background = '#00ff00';

      setTimeout(function () {
        if (id === 'demo-phase--play-throws') {
          render.phase = 'errored';
          channel.emit('playFunctionThrewException', {
            message: 'phase-order play assertion failed'
          });
          // errored の後も render は finished phase へ抜ける。
          render.phase = 'finished';
          window.__VRT_PHASE_ORDER__.push('errored-then-finished');
          return;
        }
        render.phase = 'played';
        render.phase = 'completing';
        // storyRendered は completed phase の中で出る。
        render.phase = 'completed';
        channel.emit('storyRendered', id);
        // その直後に afterEach → finished。probe が completed を観測できる
        // 窓は事実上ない。
        render.phase = 'afterEach';
        render.phase = 'finished';
        window.__VRT_PHASE_ORDER__.push('finished');
      }, 400);"#,
        );
    }
    ...
    async fn fonts_inside_a_same_origin_iframe_are_awaited_before_the_capture() {
        ...
        assert_eq!(
            ...
            "the iframe's font marker must be green — the fonts wait must \
             recurse into reachable same-origin iframes ( D)"
        );
    }
    ...
    async fn fonts_inside_a_shadow_dom_iframe_are_awaited_before_the_capture() {
        ...
        assert_eq!(
            ...
            "the shadow iframe's font marker must be green — the fonts wait must \
             descend into open shadow roots like FREEZE's freezeRoot ( 2)"
        );
    }
...
<html><frameset cols="100%"><frame src="frame.html"></frameset></html>"#,
    ...
    async fn fonts_inside_a_frameset_frame_are_awaited_before_the_capture() {
        ...
        assert_eq!(
            ...
            "the frame's font marker must be green — the fonts wait must \
             descend through frameset `frame` elements ( 5)"
        );
    }
  ...
  #box { width:100%; height:100vh; background:#00ff00; }"#,
    ...
    /// `document.defaultView` を **stateful getter** の偽物へ差し替えるバンドル。
    /// 「走査の門（[`COLLECT_DOCUMENTS_JS`] の 1 読み目）は通るが、切り離し
    /// 検知ポーリング（`readyOrDetached`）の読みで throw する」getter の
    /// 検証用。
    ///
    /// webfont（試験サーバーの `/font.ttf`——決して届かない）を使うため
    /// `fonts.ready` は pending のままで、待ちは必ず 50ms 後の検知ポーリング
    /// まで進む。読み順は 1 evaluate につき走査の門で 1 回、その後は検知
    /// ポーリング——2 回目以降を throw にすることで、走査は通り検知の読み
    /// だけが落ちる（読み順が変わると陽性対照の意味が変わるので注記。
    /// `write_stateful_fonts_getter_bundle` と同じ流儀）。
    fn write_throwing_defaultview_bundle(root: &Path) {
        write_story_html(
            root,
            r#"  html,body{margin:0;padding:0;background:#fff}
  @font-face { font-family: 'VrtTestFont'; src: url('font.ttf'); }
  .webfont-text { font-family: 'VrtTestFont', monospace; font-size: 40px; }"#,
            "",
            r#"      var text = document.createElement('div');
      text.className = 'webfont-text';
      text.textContent = 'Hamburgefonstiv';
      document.getElementById('storybook-root').appendChild(text);
      var realView = window;
      var viewReads = 0;
      Object.defineProperty(document, 'defaultView', {
        ...
        get: function () {
          viewReads++;
          if (viewReads >= 2) {
            throw new Error('stateful defaultView getter threw on a later read');
          }
          return realView;
        }
      });
    ...
    /// 【・実測】切り離し検知ポーリング（`readyOrDetached`）の
    /// `defaultView` 読みで throw する stateful getter が、原因つきの即時
    /// [`RenderError::Story`] に倒れること——検知が黙って死んで deadline まで
    /// 待つ、原因を失った Timeout にならないこと。
    ///
    /// 修正前（`if (!s.doc.defaultView)` の裸読み）は、poll の throw が
    /// setTimeout コールバック内の **uncaught 例外**となり（settle の try にも
    /// promise の reject にも乗らない）、検知 promise は永久 pending・
    /// `fonts.ready` も pending（フォント未着）のまま evaluate が返らず、
    /// story_timeout を丸ごと消費して [`FONTS_PHASES`] の timeout phase で
    /// 落ちた（旧コード赤の実測は本コミットのメッセージと報告に記録）。
    ///
    /// 修正後は帰属の印つき reject → `errors` → `ok: false` に倒れ、即座に
    /// 「defaultView の読みが throw した」と原因まで名指しで失敗する。
    /// 「切り離し扱い」へ黙って resolve する形を採らないこと（未検証のまま
    /// 撮る fail-open）も、この試験の Story 失敗（撮れないこと）が固定する。
    #[tokio::test(flavor = "multi_thread")]
    async fn a_throwing_defaultview_getter_fails_fast_with_the_reason() {
        let Some(chromium) = discover_chromium() else {
            eprintln!("SKIP a_throwing_defaultview_getter_fails_fast: no chromium");
            return;
        };
        let font = require_test_font();
        let _guard = BROWSER_LOCK.lock().await;
        ...
        let dir = tempfile::tempdir().expect("tempdir");
        write_throwing_defaultview_bundle(dir.path());
        let (addr, server_task, _hits) =
            start_font_delay_server(dir.path(), font, Duration::from_secs(3600)).await;
        ...
        let started = std::time::Instant::now();
        let err = renderer
            .render_story(&format!("http://{addr}"), "throwing-view--fonts")
            .await
            .expect_err("a throwing defaultView getter must fail the story");
        let elapsed = started.elapsed();
        renderer.close().await;
        server_task.abort();
        ...
        match err {
            RenderError::Story { message, .. } => {
                assert!(
                    message.contains("reading document.defaultView threw")
                        && message.contains("stateful defaultView getter threw"),
                    "the failure must attribute the throw to the defaultView read \
                     and carry the getter's own message, got: {message:?}"
                );
            }
            other => panic!(
                "expected an immediate reasoned Story failure for a throwing \
                 defaultView getter, got: {other:?}"
            ),
        }
        assert!(
            elapsed < Duration::from_secs(10),
            "the failure must be immediate, not a deadline exhaustion — took \
             {elapsed:?} (pre-fix the uncaught poll throw silently killed the \
             detach watch and burned the whole story_timeout)"
        );
    }
...
<script>
  ...
  ['transitionrun', 'transitionstart', 'transitionend', 'transitioncancel']
    ...
    /// （[`FONT_WARNING_MAX_FAMILIES`]+1 件）でも、載る顔ぶれと畳まれる 1 件が
    /// 順に依らず同じであること。
    /// 証明しない: 実ブラウザの Set 列挙順が実際に揺れること（揺れる余地を
    /// スクリプト側の契約にしないため、Rust 側で閉じるのが本修正の趣旨）。
    ///
    /// 旧コード赤: sort 導入前は逆順入力が逆順のまま列挙され、下の両
    /// assert_eq が落ちた（実測は本コミットの報告に記録）。
    #[test]
    fn fonts_verdict_warning_is_independent_of_family_order() {
        let warn = |families: &[&str]| {
            let raw = format!(
                r#"{{"ok":true,"status":"loaded","failed":{},"errors":[]}}"#,
                serde_json::to_string(families).unwrap()
            );
            let value = serde_json::Value::String(raw);
            fonts_verdict(Some(&value), "s")
                .expect("ok:true with failures must still pass")
                .expect("failed fonts must produce a warning")
        };
        ...
        assert_eq!(
            ...
            warn(&["Gamma", "Beta", "Alpha"]),
            "the warning must not depend on the arrival order of families"
        );

        // 打ち切り境界（MAX=5 で 6 件）: 逆順でも「載る 5 件・畳まれる 1 件」が
        // 同じ——順が生きたままだと、どの family の名が消えるかまで揺れる。
        let six = ["FamA", "FamB", "FamC", "FamD", "FamE", "FamF"];
        let mut six_rev = six;
        six_rev.reverse();
        let forward = warn(&six);
        assert_eq!(
            forward,
            warn(&six_rev),
            "the truncation boundary must also be order-independent"
        );
        assert!(
            forward.contains("FamA, FamB, FamC, FamD, FamE")
                && forward.contains("and 1 more")
                && !forward.contains("FamF"),
            "sorting must decide which family folds, got: {forward}"
        );
    }
    ...
    /// 観測できるのは定常値 `finished` だけで、世代の合う rendered 印を持つ
    /// **正常な story が一つも READY にならず**、全て 30 秒の Timeout に倒れて
    /// いた（READY probe を抜き出して `phase: 'finished'`・rendered 印あり・
    /// root に子ありで評価すると `pending` を返した）。
    #[tokio::test(flavor = "multi_thread")]
    async fn a_story_that_settles_in_the_finished_phase_is_captured() {
        let Some(chromium) = discover_chromium() else {
            eprintln!("SKIP a_story_that_settles_in_the_finished_phase_is_captured: no chromium");
            return;
        };
        let _guard = BROWSER_LOCK.lock().await;
        ...
        let started = std::time::Instant::now();
        let png = renderer
            .render_story(&server.base_url(), "demo-phase--passes")
            .await
            .expect("a story that settles in the finished phase must be captured")
            .png;
        let elapsed = started.elapsed();
        renderer.close().await;
        ...
        assert!(
            ...
            "capture must wait for the play to finish, took {elapsed:?}"
        );
        let image = decode_png(&png);
        let center = image.get_pixel(160, 120);
        assert_eq!((center[0], center[1], center[2]), (0, 255, 0));
    ...
    /// 同じ phase 順で play が投げたときは、その後 render が `finished` phase へ
    /// 抜けても撮らない——世代内の先着 error が完了 phase に上書きされない。
    #[tokio::test(flavor = "multi_thread")]
    async fn a_play_failure_is_not_masked_by_the_finished_phase() {
        let Some(chromium) = discover_chromium() else {
            eprintln!("SKIP a_play_failure_is_not_masked_by_the_finished_phase: no chromium");
            return;
        };
        let _guard = BROWSER_LOCK.lock().await;
        ...
        let err = renderer
            .render_story(&server.base_url(), "demo-phase--play-throws")
            .await
            .expect_err("a play failure must stop the capture even after the render settles");
        renderer.close().await;
